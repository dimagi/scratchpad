...
- [ ] **Q:** For the data freshness requirement, which implementation approach should be used — synchronous write to the project DB as part of form submission, or a client/server memory cache merged with endpoint results? | *Raised:* 2026-03-10 | *By:* Woody | *Docs:* `additional_endpoint_requirements.md`
  ...
  - *UX consideration (2026-03-12, Woody):* If async pillows remain the baseline, could we partially satisfy the requirement through UX rather than data architecture — e.g., a staleness banner ("results may not reflect your most recent submission") or app navigation restrictions that prevent users from immediately returning to a results list after form submission?
...
- [ ] **Q:** How should cross-case-type joined structures be defined and managed for app builders? Options: (1) **raw SQL** scoped to the project database, authored directly by app builders; (2) **GUI editor** (similar in spirit to the query builder for filter conditions); (3) **developer-hardcoded** definitions. Note: the query builder is needed regardless of which approach is chosen — it handles the filter/condition layer on top of whatever structure is defined. | *Raised:* 2026-03-12 | *By:* Woody | *Docs:* `project_db_design.md`
